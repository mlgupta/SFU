 ...
 *                                                                           *
 *****************************************************************************
 * $Id: SearchJobs.java,v 1.3 2006/02/14 10:30:06 ias Exp $
 *****************************************************************************
 */
...
public class SearchJobs {
  private static final String[] jobGroupNames = { "MAIL", "FAX", "UPLOAD" };

  private static final String[] defaultColumnOrder =
  { "executionTime", "creatorName" };
  ...
  /**
     ...
     */
  public ArrayList findJobs(Scheduler scheduler, String[] orderColumns,
                            String creatorName, String jobType,
                            Date fromCreateTime, Date toCreateTime,
                            Date fromExecutionTime, Date toExecutionTime,
    ...
    try {
      for (int index = 0; index < jobGroupNames.length; index++) {
        ...
        for (int jIndex = 0; jIndex < jobNames.length; jIndex++) {
          Trigger trigger[] =
            scheduler.getTriggersOfJob(jobNames[jIndex], jobGroupNames[index]);

          JobDataMap jobData =
            scheduler.getJobDetail(jobNames[jIndex], jobGroupNames[index])
            .getJobDataMap();

          if (jobFilter
              .filterJob(jobData, creatorName, jobType, fromCreateTime,
                                  toCreateTime, fromExecutionTime,
                                  toExecutionTime)) {

            String sJobName = jobNames[jIndex];
            String sCreatorName =
              jobData.getString(SchedulerConstants.JOB_CREATOR);
            String sJobType = jobData.getString(SchedulerConstants.JOB_TYPE);
            String sRetrialcount =
              jobData.getString(SchedulerConstants.RETRIAL_COUNT);
            String sRetrialInterval =
              jobData.getString(SchedulerConstants.RETRIAL_INTERVAL);
            String sMaxCount = jobData.getString(SchedulerConstants.MAX_COUNT);
            Date sCreateTime =
              (Date)jobData.get(SchedulerConstants.CREATE_TIME);
            Date sExecutionTime =
              (Date)jobData.get(SchedulerConstants.EXECUTION_TIME);
            String sErrorMessage = null;
            if (trigger == null || trigger.length == 0) {
              //sErrorMessage = "Job Dispatched";
              sErrorMessage = jobData.getString(SchedulerConstants.ERROR_MESG).equalsIgnoreCase("NA")?"Job Dispatched":"Job Failed";
            } else {
              sErrorMessage = jobData.getString(SchedulerConstants.ERROR_MESG);
            }
            JobListBean jobBean =
              new JobListBean(sJobName, sCreatorName, sJobType, sCreateTime,
                                                  sExecutionTime,
                                                  sRetrialcount,
                                                  sRetrialInterval, sMaxCount,
                                                  sErrorMessage);
            jobList.add(jobBean);
          }
